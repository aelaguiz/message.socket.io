...
/**
 * Walks the event path and calls the handler function
 */
	...
	if(undefined === queryObject) {
		...
		if (undefined === targetSpec) {
			...
			if (undefined === targetFunctionName) {
				callback(new Error("Target is not an event handler " + path), requestIndex);
			}
			else {
				targetFunction = queryObject[targetFunctionName];

				if(undefined === targetFunction) {
					callback(new Error("Event api not implemented " + path), requestIndex);
				}
				else {
...
/**
 * Walks the query path and calls the handler function
 */
